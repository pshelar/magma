...
class PipelinedRpcServicer(pipelined_pb2_grpc.PipelinedServicer):
    ...
    def _install_flows_gx(self, request: ActivateFlowsRequest,
        ...
        enforcement_stats_res = self._activate_rules_in_enforcement_stats(
            request.sid.id, request.msisdn, ip_address, request.apn_ambr, request.rule_ids,
        ...
        enforcement_res = self._activate_rules_in_enforcement(
            request.sid.id, request.msisdn, ip_address, request.apn_ambr, static_rule_ids,
            dynamic_rules)
    ...
    def _activate_rules_in_enforcement_stats(self, imsi: str,
        ...
        enforcement_stats_res = self._enforcement_stats.activate_rules(
            imsi, msisdn, ip_addr, apn_ambr, static_rule_ids, dynamic_rules)
        _report_enforcement_stats_failures(enforcement_stats_res, imsi)
        return enforcement_stats_res

    def _activate_rules_in_enforcement(self, imsi: str, msisdn: bytes, ip_addr: IPAddress,
                                       apn_ambr: AggregatedMaximumBitrate,
                                       static_rule_ids: List[str],
                                       dynamic_rules: List[PolicyRule]
                                       ) -> ActivateFlowsResult:
    ...
    def _activate_rules_in_gy(self, imsi: str, msisdn: bytes, ip_addr: IPAddress,
                              apn_ambr: AggregatedMaximumBitrate,
                              static_rule_ids: List[str],
                              dynamic_rules: List[PolicyRule]
                              ) -> ActivateFlowsResult:
        gy_res = self._gy_app.activate_rules(imsi, msisdn, ip_addr, apn_ambr,
                                             static_rule_ids,
                                             dynamic_rules)
        # TODO: add metrics
        return gy_res
