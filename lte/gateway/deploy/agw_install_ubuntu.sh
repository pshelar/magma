...
INTERFACES=$(ip -br a)
if [[ $1 != "$CLOUD_INSTALL" ]] && ( [[ ! $INTERFACES == *'eth0'*  ]] || ! grep -q 'GRUB_CMDLINE_LINUX="net.ifnames=0 biosdevname=0"' /etc/default/grub); then
...
if [ "$MAGMA_INSTALLED" != "$SUCCESS_MESSAGE" ]; then
  ...
  apt-get update
  apt-get -y install curl make virtualenv zip rsync git software-properties-common python3-pip python3-dev
