...
int openflow_add_s8_tunnel(struct in_addr ue, struct in6_addr* ue_ipv6, int vlan,
      struct in_addr enb, struct in_addr pgw,
      uint32_t i_tei, uint32_t o_tei, Imsi_t imsi, struct ip_flow_dl* flow_dl,
      uint32_t flow_precedence_dl) {
...
return openflow_controller_add_gtp_s8_tunnel(
      ue, ue_ipv6, vlan, enb, pgw, i_tei, o_tei, (const char*) imsi.digit, flow_dl,
      flow_precedence_dl, enb_portno, pgw_portno);
}

int openflow_del_s8_tunnel(struct in_addr enb, struct in_addr pgw,
