...
static uint32_t create_gtp_port(struct in_addr enb_addr, char port_name[]) {
  ...
  rc = snprintf(
      ...
      "sudo ovs-vsctl add-port gtp_br0 %s -- set Interface %s type=%s "
      "options:remote_ip=%s",
...
const struct gtp_tunnel_ops* gtp_tunnel_ops_init_openflow(void) {
  ...
  int initial_size = INIT_GTP_TABLE_SIZE;

  gtp_portno_rec.arr = calloc(initial_size, sizeof(struct gtp_portno));
  assert(gtp_portno_rec.arr != NULL);
  gtp_portno_rec.size = initial_size;
